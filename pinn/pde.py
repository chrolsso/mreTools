...
class WaveEquation(object):
    ...
    def from_name(cls, pde_name, **kwargs):
        #If from_name is a class method, it can be called on the class itself (pde) without first constructing an instance of the class. Class methods have access to the class itself (via the cls parameter) rather than instances of the class. 
        pde_name = pde_name.lower()  # CHOSSES EQUATION TO IMPLEMENT
    ...
    def traction_forces(self, x, u, mu):
        ...
        grad_u = jacobian(u, x, dim=1) #Continuous Jacobian operator, maps a vector field to a tensor field
        if self.detach:
            grad_u = grad_u.detach() #To reduce computational cost, we detach the gradient before processing any minibatch so that the gradient computation of the hidden state is always limited to the time steps in one minibatch:grad_u is detached from the computation graph, so any further computation involving it won't be used for calculating gradients during backpropagation.: weights won't be updates according to this derivative
        # Detaching u effectively treats it as a constant during backpropagation, which may or may not be appropriate depending on the application.
        ...
        stress = mu_strain + lambda_strain # The material properties (mu and lambda_) are used to compute the stress tensor based on the strain tensor

        # is this divergence correct if we detach u?
        # the divergence calculation with a detached u would be correct in the sense that it calculates the divergence with u treated as constant. However, if u needs to be part of the computation graph for gradient computation, detaching it might lead to incorrect gradients during backpropagation.
...
class HeteroEquation(WaveEquation):
    ...
    def traction_forces(self, x, u, mu): #FUNCTION 
...
class CompressEquation(WaveEquation):
    """
    Represents a compress equation for wave propagation.

    This class inherits from the WaveEquation class and provides additional methods for calculating traction forces.

    Attributes:
        detach (bool): Flag indicating whether to detach tensors during calculations.
        lambda_ (float): Lambda coefficient for the equation.
    """

    def traction_forces(self, x, u, mu):
        """
        Calculates the traction forces for the compress equation.

        Args:
            x (torch.Tensor): Input tensor.
            u (torch.Tensor): Input tensor.
            mu (float): Mu coefficient for the equation.

        Returns:
            torch.Tensor: Traction forces tensor.
        """
