...
class MREPINNData(deepxde.data.Data):
    ...
    def losses(self, targets, outputs, loss_fn, inputs, model, aux=None):
        ...
        # mask the image before calculating the loss so that only the loss in the region of interest is considered
        u_loss = 0.0
        try:
            bin_mask = self.example.bin_mask.field.values()
            print(bin_mask.shape)
            print(u_pred.shape)
            u_pred_masked = u_pred * bin_mask
            u_true_masked = u_true * bin_mask

            u_loss  = loss_fn(u_true_masked, u_pred_masked)
        except:
